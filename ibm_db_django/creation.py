...
if not _IS_JYTHON:
    try:
        ...
        raise ImportError(
            "ibm_db module not found. Install ibm_db module from "
            "http://code.google.com/p/ibm-db/. Error: %s" %
            e)
...
class DatabaseCreation (BaseDatabaseCreation):
    ...
    def sql_indexes_for_field(self, model, f, style):
        ...
        tablespace_sql = ''
        i = 0
        conn_dbms_name = getattr(self.connection.connection, dbms_name, '')
        if DJ_VER >= (1, 8) and 'DB2' not in conn_dbms_name or conn_dbms_name != 'DB2':
            if len(model._meta.unique_together_index) != 0:
                for unique_together_index in model._meta.unique_together_index:
                    ...
                    column_list.extend([
                        truncate_name("%s%s" % (
                            self.psudo_column_prefix, "_".join(column_list)), max_name_length)])
                    output.extend([
                        style.SQL_KEYWORD('CREATE UNIQUE INDEX') + ' ' +
            ...
            if f.unique_index:
                ...
                output.extend([
                    style.SQL_KEYWORD(cisql) +
                    ' ' +
                    style.SQL_TABLE(qn('%s_%s' %
                    ...
                    "(%s, %s )" % (
                        ...
                        style.SQL_FIELD(qn(truncate_name((
                            self.psudo_column_prefix + f.column), max_name_length)))) +
                    "%s;" %
                    tablespace_sql])
                return output
    ...
    # Method to create and return test database, before creating test
    # database it takes confirmation from user. If test database already
    # exists then it takes confirmation from user to recreate that database
    # . If create test database not supported in current scenario then it
    # takes confirmation from user to use settings file's database name as
    # test database For Jython this method prepare the settings file's
    # database. First it drops the tables from the database,then create
    ...
    def create_test_db(self, verbosity=0, autoclobber=False, keepdb=False, serialize=False):
        ...
        if not _IS_JYTHON:
            ...
            kwargs['database'] = truncate_name("%s%s" % (
                TEST_DBNAME_PREFIX, old_database), max_db_name_length)
            ...
            if autoclobber or confirm == 'yes':
                try:
                    ...
                    if (message.find('Not supported:') != -1):
                        confirm = raw_input(
                            "Not able to create test database, %s. Type yes to use %s "
                            "as test database, or no to exit" %
                        ...
                        if(message != '') & (index != -1):
                            ...
                            if(error_code != '-1005'):
                                ...
                                confirm = raw_input(
                                    "\nTest database: %s already exist. Type yes to "
                                    "recreate it, or no to exit" % (kwargs.get('database')))
                                if autoclobber or confirm == 'yes':
                                    if verbosity > 1:
                ...
                confirm = raw_input(
                    "Wants to use %s as test database, Type yes to use it as "
                    "test database or no to exit" % (old_database))
                if confirm == 'yes':
                    kwargs['database'] = old_database
                    self.__clean_up(self.connection.cursor())
                    self.connection._commit()
        ...
        if(DJ_VER <= (1, 1)):
            ...
            if (DJ_VER < (1, 7)):
                ...
                call_command(
                    ...
                    interactive=False,
                    load_initial_data=False)
            # We need to then do a flush to ensure that any data installed by custom
    ...
    def sql_create_model(self, model, style, known_models=set()):
        if getattr(self.connection.connection, dbms_name) != 'DB2':
            ...
            sql, references = super(
                DatabaseCreation, self).sql_create_model(
    ...
    def __add_pseudokey_column(self, style, cursor, table_name, pk_name, column_list):
        qn = self.connection.ops.quote_name
        max_name_length = self.connection.ops.max_name_length()
        ...
        sql = style.SQL_KEYWORD('ALTER TABLE ') + \
            ...
            style.SQL_FIELD(qn(truncate_name("%s%s" % (
                self.psudo_column_prefix, "_".join(column_list)), max_name_length))) + \
