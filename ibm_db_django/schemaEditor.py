...
class DB2SchemaEditor(BaseDatabaseSchemaEditor):
    ...
    def alter_field(self, model, old_field, new_field, strict=False):
        ...
        if(djangoVersion[0:2] < (1, 9)):
            if old_field.rel is not None and hasattr(old_field.rel, 'through'):
                rel_condition = (
                    old_field.rel.through and new_field.rel.through and old_field.rel.through._meta.auto_created and new_field.rel.through._meta.auto_created)
            else:
                rel_condition = False
        else:
        ...
        if (old_db_field_type != new_db_field_type) and (isinstance(old_field,
                                                                    (models.AutoField, models.TextField)) or isinstance(new_field, models.TextField)):
            if old_field.primary_key and new_field.primary_key:
                rebuild_incomming_fk = True
                if isinstance(
                        old_field,
                        models.AutoField) and isinstance(
                        new_field,
                        models.IntegerField):
                    pass
                else:
        ...
        if alter_field_name or alter_field_data_type:
            ...
            if old_field.primary_key and new_field.primary_key:
                ...
                for incoming_fks in old_field.model._meta.get_fields():
                    fk_names = self._constraint_names(
                        incoming_fks.model, [
                            incoming_fks.field.column], foreign_key=True)
                    for fk_name in fk_names:
                        self.execute(
                            self.sql_delete_fk % {
            ...
            if alter_field_data_type:
                ...
                if (old_field.default is not None) and (
                        old_field.has_default()) and (old_default is not None):
                ...
                self.execute(
                    self.sql_create_fk %
                    {
                        'table': self.quote_name(
                            model._meta.db_table), 'name': self._create_index_name(
                            model, [
                                new_field.column], suffix="_fk"), 'column': self.quote_name(
                            new_field.column), 'to_table': self.quote_name(
                            new_field.remote_field.model._meta.db_table), 'to_column': self.quote_name(
        ...
        if rebuild_incomming_fk:
            for inc_rel in new_field.model._meta.get_all_related_objects():
                self.execute(
                    ...
                    {
                        'table': self.quote_name(
                            inc_rel.model._meta.db_table), 'name': self._create_index_name(
                            inc_rel.model, [
                                inc_rel.field.column], suffix="_fk"), 'column': self.quote_name(
                            inc_rel.field.column), 'to_table': self.quote_name(
                            model._meta.db_table), 'to_column': self.quote_name(
                                new_field.column), })
    ...
    def alterFieldDataTypeByRemaking(self, model, old_field, new_field, strict):
        ...
        tmp_new_field.column = truncate_name(
            "%s%s" %
            (self.psudo_column_prefix,
             tmp_new_field.column),
    ...
    def add_field(self, model, field):
        ...
        if notnull or unique or p_key:
            ...
            if p_key:
                ...
                for other_pk in cur.connection.primary_keys(
                        True, cur.connection.get_current_schema(), model._meta.db_table):
                    self.execute(
                        self.sql_delete_pk % {
                            'table': self.quote_name(model._meta.db_table),
    ...
    def _alter_many_to_many(self, model, old_field, new_field, strict):
        ...
        if(djangoVersion[0:2] < (1, 9)):
            if ((old_field.rel is not None and hasattr(old_field.rel, 'through')) and
                ...
                rel_old_field = old_field.rel.through._meta.get_field(
                    old_field.m2m_reverse_field_name())[0]
                rel_new_field = new_field.rel.through._meta.get_field(
                    new_field.m2m_reverse_field_name())[0]
            else:
                rel_old_field = None
                rel_new_field = None
        else:
            ...
            self.execute(
                self.sql_create_pk % {
                    ...
                    'columns': ', '.join(
                        column.replace(
                            old_field.column,
                            new_field.column) for column in columns)})
