...
if not _IS_JYTHON:
    try:
        ...
        raise ImportError(
            "ibm_db module not found. Install ibm_db module from http://code.google.com/p/ibm-db/. Error: %s" %
            e)
else:
...
class DatabaseIntrospection(BaseDatabaseIntrospection):
    ...
    def get_table_list(self, cursor):
        ...
        if not _IS_JYTHON:
            ...
            # tables(String catalog, String schemaPattern, String tableNamePattern,
            # String[] types) gives a description of tables available in a catalog
    ...
    def get_relations(self, cursor, table_name):
        ...
        if not _IS_JYTHON:
            ...
            for fk in cursor.fetchall():
                # fk[2] is primary key table name, fk[3] is primary key column name, fk[7]
                # is foreign key column name being exported
    ...
    def __get_col_index(self, cursor, schema, table_name, col_name):
        if not _IS_JYTHON:
            ...
            cursor.execute("select current_schema from sysibm.sysdummy1")
            schema = cursor.fetchone()[0]
            # columns(String catalog, String schemaPattern, String tableNamePattern,
            # String columnNamePattern) gives a description of table columns available
    ...
    def get_indexes(self, cursor, table_name):
        ...
        if not _IS_JYTHON:
            ...
            cursor.execute("select current_schema from sysibm.sysdummy1")
            schema = cursor.fetchone()[0]
            # statistics(String catalog, String schema, String table, boolean unique,
            # boolean approximate) returns a description of a table's indices and
            # statistics.
            ...
            # primarykeys(String catalog, String schema, String table) gives a
            # description of a table's primary key columns
